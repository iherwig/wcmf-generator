...
public class Generator {
	...
	public static void main(String[] arguments) {
		if (arguments.length == 0)
			...
			try {
				...
				try {
					String genClass = '/'
							+ Generator.class.getName().replace('.', '/')
							+ ".class";
					...
					if ("jar" == genClassURL.getProtocol()) {
						...
						prefix = prefix.substring(0,
								prefix.lastIndexOf("!/") + 2);
					}
					JarURLConnection conn = (JarURLConnection) new URL(prefix)
							.openConnection();
					...
					if (manifest != null) {
						...
						Attributes attrs = (Attributes) manifest
								.getMainAttributes();
						for (Iterator it = attrs.keySet().iterator(); it
								.hasNext();) {
							Attributes.Name attrName = (Attributes.Name) it
									.next();
						...
						Logger
								.info("Running ChronosGenerator " + version
										+ " ...");
					}
				} catch (Exception e) {
				}
				...
				new WorkflowRunner().run(wfFile, new NullProgressMonitor(),
	 ...
	 */
	public static XtendFacade createXtendFacade(String xTendFile,
			Profile[] usedProfiles) {
