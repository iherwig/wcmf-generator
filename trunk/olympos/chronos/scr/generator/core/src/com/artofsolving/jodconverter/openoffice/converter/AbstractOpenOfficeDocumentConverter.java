...
public abstract class AbstractOpenOfficeDocumentConverter implements DocumentConverter {
    ...
    protected void refreshDocument(XComponent document) throws IndexOutOfBoundsException, WrappedTargetException {
		...
		if (indexesSupplier != null) {
			XIndexAccess indexAccess = indexesSupplier.getDocumentIndexes();
			
			for (int i = 0; i < indexAccess.getCount(); i++) {
				Object currIndex = indexAccess.getByIndex(i);
				
				XDocumentIndex index = (XDocumentIndex) UnoRuntime.queryInterface(XDocumentIndex.class, currIndex);
