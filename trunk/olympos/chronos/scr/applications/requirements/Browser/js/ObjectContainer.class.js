...
ObjectContainer = function(){
    ...
    /**
     * Two lists containing the object information for Package weight (treemap) and Package tree (spacetree)
     */
 ...
 */
ObjectContainer.prototype.loadModels = function(){
    var self = this;
    
    this.models['name'] = [];
    this.models['oid'] = [];
    uwm.persistency.Persistency.getInstance().loadChildren('root', function(options, data){
        self.handleLoadedModels(options, data);
    }, function(){
    });
}

ObjectContainer.prototype.handleLoadedModels = function(options, data){
    for (i in data.objects) {
        if (!(data.objects[i] instanceof Function)) {
            this.models.name.push(data.objects[i].text);
            this.models.oid.push(data.objects[i].oid);
        }
    }
 ...
 */
ObjectContainer.prototype.getTreeList = function(rootOid){
 ...
 */
ObjectContainer.prototype.getWeightList = function(rootOid){
 ...
 * @type String
 */
ObjectContainer.prototype.setModelOid = function(){
    var result = false;
    for (var i = 0; i < this.models['name'].length; i++) {
        if (this.models['name'][i] == this.selectedModelName) {
            result = true;
            this.selectedModel = this.models['oid'][i];
        }
    }
    return result;
}
