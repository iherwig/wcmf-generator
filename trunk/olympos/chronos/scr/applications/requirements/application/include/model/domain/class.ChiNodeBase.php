...
class ChiNodeBase extends ChiBase
{
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'Package')
      {
        // for every NMChiNodeChiNode we have to load the Package 
        $this->loadChildren('NMChiNodeChiNode');
        $children = parent::getChildrenEx(null, 'NMChiNodeChiNode', array('fk_chinodetarget_id' => $this->getDBID()), null);
      ...
      if ($type == 'Package')
      {
        // for every NMChiNodeChiNode we have to load the Package 
        $this->loadChildren('NMChiNodeChiNode');
        $children = parent::getChildrenEx(null, 'NMChiNodeChiNode', array('fk_chinodesource_id' => $this->getDBID()), null);
        $persistenceFacade = &PersistenceFacade::getInstance();
        $grandChildren = array();
        for($i=0; $i<sizeof($children); $i++)
        {
    ...
    function getChildrenEx($oid, $type, $values, $properties)
    {
      ...
      {
        // for every NMChiNodeChiNode we have to get the Package parents 
        $children = parent::getChildrenEx(null, 'NMChiNodeChiNode', array('fk_chinodetarget_id' => $this->getDBID()), null);
        $grandChildren = array();
        $persistenceFacade = &PersistenceFacade::getInstance();
        for($i=0; $i<sizeof($children); $i++)
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      ...
      {
        // for every Package we have to insert a NMChiNodeChiNode
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMChiNodeChiNode', array('fk_chinodetarget_id' => $this->getDBID(), 'fk_package_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMChiNodeChiNode', BUILDTYPE_SINGLE);
          $associationNode->setChiNode($this);
      ...
      if ($child != null && $child->getType() == 'Package')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMChiNodeChiNode', array('fk_chinodesource_id' => $this->getDBID(), 'fk_package_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMChiNodeChiNode', BUILDTYPE_SINGLE);
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      ...
      {
        // for every Package we have to delete the NMChiNodeChiNode
