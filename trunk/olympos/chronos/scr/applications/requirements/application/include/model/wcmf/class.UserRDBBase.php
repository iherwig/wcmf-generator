...
class UserRDBBase extends User
{
    ...
    function getChildren()
    {
      ...
      $childrenNMUserRole = parent::getChildrenEx(null, 'NMUserRole', array('fk_user_id' => $this->getDBID()), null);
      for($i=0; $i<sizeof($childrenNMUserRole); $i++)
      {
        $grandChildren = $childrenNMUserRole[$i]->getParentsEx(null, 'RoleRDB', null, null);
    ...
    function getChildrenEx($oid, $type, $values, $properties)
    {
      ...
      if ($type == 'RoleRDB' || PersistenceFacade::getOIDParameter($oid, 'type') == 'RoleRDB')
      {
        ...
        for($i=0; $i<sizeof($children); $i++)
        {
          $grandChildrenParents = $children[$i]->getRoleRDBList();
   ...
   /**
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'RoleRDB')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('NMUserRole', array('fk_user_id' => $this->getDBID(), 'fk_role_id' => PersistenceFacade::getOIDParameter($childOID, 'id')));
        if ($relOID != null)
        {
