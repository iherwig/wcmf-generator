...
class ChiBusinessUseCaseBase extends ChiBase
{
    ...
    function getValueDisplayName($name, $type=null)
    {
      ...
      if ($name == 'Composition') $displayName = Message::get("Composition");
    ...
    function getValueDescription($name, $type=null)
    {
      ...
      if ($name == 'Composition') $description = Message::get("");
    ...
    function getComposition($unconverted=false)
    {
      if ($unconverted)
        return $this->getUnconvertedValue('Composition', DATATYPE_IGNORE);
      else
        return $this->getValue('Composition', DATATYPE_IGNORE);
    }
    function setComposition($Composition)
    {
      return $this->setValue('Composition', $Composition, DATATYPE_IGNORE);
    }
    ...
    {
      $fkValue = $this->getValue('Composition', DATATYPE_IGNORE);
      if ($fkValue != null)
    ...
    {
      if ($node != null)
        $this->setValue('Composition', $node->getDBID(), DATATYPE_IGNORE);
    }
    ...
    {
      return $this->getChildrenEx(null, 'ActivitySet', array('Aggregation' => $this->getDBID()), null, false);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'NMUCActor', array('Association' => $this->getDBID()), null, false);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'ChiController', array('Realization' => $this->getDBID()), null, false);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'Figure', array('fk_chibusinessusecase_id' => $this->getDBID()), null, false);
    }
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'Actor')
      {
        ...
        $children = parent::getChildrenEx(null, 'NMUCActor', array('Association' => $this->getDBID()), null, false);
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      if ($child != null && $child->getType() == 'Actor')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'Association' => $child->getDBID()));
        if ($relOID == null)
        {
      ...
      if ($child != null && $child->getType() == 'ChiBusinessPartner')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'fk_chibusinesspartner_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiBusinessPartnerPassive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'fk_chibusinesspartnerpassive_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiBusinessPartnerActive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'fk_chibusinesspartneractive_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiWorker')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'fk_chiworker_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiWorkerInternal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'fk_chiworkerinternal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiWorkerExternal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('Association' => $this->getDBID(), 'fk_chiworkerexternal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Actor')
      {
        ...
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'Association' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiBusinessPartner')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'fk_chibusinesspartner_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiBusinessPartnerPassive')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'fk_chibusinesspartnerpassive_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiBusinessPartnerActive')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'fk_chibusinesspartneractive_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiWorker')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'fk_chiworker_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiWorkerInternal')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'fk_chiworkerinternal_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiWorkerExternal')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('Association' => $this->getDBID(), 'fk_chiworkerexternal_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
