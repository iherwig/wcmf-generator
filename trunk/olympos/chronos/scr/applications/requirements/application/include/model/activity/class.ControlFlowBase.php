...
class ControlFlowBase extends EntityBaseExtended
{
    ...
    {
      if ($type == null)
        parent::EntityBaseExtended($oid, 'ControlFlow');
      else
    ...
    function getASControlFlowTargetEndOID()
    {
    ...
    function setASControlFlowTargetEnd($node)
    {
      if ($node != null) {
        if (!is_a($node, 'node') && !is_a($node, 'Node')) {
          $args = func_get_args();
          call_user_func_array(array(parent, 'setASControlFlowTargetEnd'), $args);
        }
        else {
    ...
    function getASControlFlowTargetEndParents()
    {
      return $this->getParentsEx(null, 'ActivitySend', null, null);
    }
