...
class ChiBusinessPartnerPassiveBase extends ChiBusinessPartner
{
    ...
    {
      return $this->getChildrenEx(null, 'ChiBusinessPartnerPassive', array('Generalization' => $this->getDBID()), null, false);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'Figure', array('fk_chibusinesspartnerpassive_id' => $this->getDBID()), null, false);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'NMUCActor', array('fk_chibusinesspartnerpassive_id' => $this->getDBID()), null, false);
    }
    /**
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'ChiBusinessUseCase')
      {
        ...
        $children = parent::getChildrenEx(null, 'NMUCActor', array('fk_chibusinesspartnerpassive_id' => $this->getDBID()), null, false);
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      if ($child != null && $child->getType() == 'ChiBusinessUseCase')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chibusinesspartnerpassive_id' => $this->getDBID(), 'Association' => $child->getDBID()));
        if ($relOID == null)
        {
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiBusinessUseCase')
      {
        ...
        $associationNodeConstraint = array('NMUCActor', array('fk_chibusinesspartnerpassive_id' => $this->getDBID(), 'Association' => $ids[0]));
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ChiBusinessUseCaseCore')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('NMUCActor', array('fk_chibusinesspartnerpassive_id' => $this->getDBID(), 'fk_chibusinessusecasecore_id' => $ids[0]));
        $associationNode = &$this->getFirstChild('NMUCActor', $associationNodeConstraint, null);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
