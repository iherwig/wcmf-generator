...
class ChiWorkerBase extends Actor
{
    ...
    {
      return $this->getChildrenEx(null, 'NMUCActor', array('fk_chiworker_id' => $this->getDBID()), null);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'Figure', array('fk_chiworker_id' => $this->getDBID()), null);
    }
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'Actor')
      {
        ...
        $children = parent::getChildrenEx(null, 'NMUCActor', array('fk_chiworker_id' => $this->getDBID()), null);
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      if ($child != null && $child->getType() == 'Actor')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chiworker_id' => $this->getDBID(), 'fk_actor_id' => $child->getDBID()));
        if ($relOID == null)
        {
      ...
      if ($child != null && $child->getType() == 'ChiBusinessPartner')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chiworker_id' => $this->getDBID(), 'fk_chibusinesspartner_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiBusinessPartnerPassive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chiworker_id' => $this->getDBID(), 'fk_chibusinesspartnerpassive_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiBusinessPartnerActive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chiworker_id' => $this->getDBID(), 'fk_chibusinesspartneractive_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiWorkerInternal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chiworker_id' => $this->getDBID(), 'fk_chiworkerinternal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ChiWorkerExternal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('NMUCActor', array('fk_chiworker_id' => $this->getDBID(), 'fk_chiworkerexternal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('NMUCActor', BUILDTYPE_SINGLE);
