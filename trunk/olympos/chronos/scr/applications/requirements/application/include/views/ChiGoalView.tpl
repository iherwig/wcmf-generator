<!--
  This file was generated by wCMFGenerator 3.0.0017 from requirements.uml on Tue Aug 25 10:00:59 CEST 2009. 
  Manual modifications should be placed inside the protected regions.
-->
<!-- PROTECTED REGION ID(application/include/views/ChiGoalView.tpl/Body) ENABLED START -->
...
{if $viewMode == 'detail'}
    ...
    // grid initialization
    grids['ChiGoalParent'].init('{$template->getObjectDisplayName()}', 'ChiGoal', "{$obfuscator->obfuscate($nodeUtil->getParentQuery('ChiGoal', $node))}", columDefs['ChiGoalParent'], {ldelim}paging:false, autoheight:true, singleSelect:true, ddRows:{$ddRows}{rdelim}, [new wcmf.grid.EditAction(){if $template->getProperty('composition') == false}, new wcmf.grid.DeleteAction(){/if}], buttonDefs['ChiGoalParent']);
    ...
    // grid initialization
    grids['ChiRequirementChild'].init('{$realSubject->getObjectDisplayName()}', 'ChiRequirement', '{$obfuscator->obfuscate($nodeUtil->getChildQuery($node, 'ChiRequirement'))}', columDefs['ChiRequirementChild'], {ldelim}paging:true, autoheight:true, singleSelect:true, ddRows:{$ddRows}{rdelim}, [new wcmf.grid.EditAction(), {if $template->getProperty('canCreate')}new wcmf.grid.DuplicateAction(), {/if}new wcmf.grid.DeleteAction()], buttonDefs['ChiRequirementChild'], {ldelim}poid:'{$node->getOID()}'{rdelim});
...
<div class="contentblock">
	...
	<span class="dottedSeparator"></span>
	<span class="left" title="{$node->getValueDescription('GoalType')}">{$node->getValueDisplayName('GoalType')}</span>
	<span class="right">{$nodeUtil->getInputControl($node, 'GoalType')}</span>
	<span class="dottedSeparator"></span>
