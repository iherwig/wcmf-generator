 ...
 */
cwb.persistency.LongTask = function(call, iFrameId) {
	this.call = call;
 ...
 *          Parameters: data The data returned from the last server call
 */
cwb.persistency.LongTask.prototype.run = function(processHandler, successHandler, errorHandler) {
...
cwb.persistency.LongTask.prototype.jsonSuccess = function(options, data) {
	...
	if (stepNumber > numberOfSteps) {
		...
		if (iFrame) {
			...
			iFrame.set({src:'../application/main.php?response_format=HTML&usr_action=continue&controller='+controller+'&sid='+cwb.Session.getInstance().getSid()});
			if (this.successHandler instanceof Function) {
				this.successHandler(data);
			}
		}
