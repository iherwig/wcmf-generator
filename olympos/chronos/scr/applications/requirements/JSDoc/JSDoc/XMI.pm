...
sub output {
    ...
    my @descriptions = $self->get_descriptions(@packages);
...
sub get_descriptions {
	...
	my @descriptions = ();
	for my $class (@{$packages->{classes}}) {
   		my %local_hash = ();
		$local_hash{id} = $class->{classid};
  		$local_hash{description} = $class->{classdescription};
  		push @descriptions, {%local_hash};

   		for my $method (@{$class->{methods}}) {
       		my %local_hash = ();
       		$local_hash{id} = $method->{methodid};
       		$local_hash{description} = $method->{methoddescription};
       		push @descriptions, {%local_hash};
   		}
   		for my $attribute (@{$class->{attributes}}) {
       		my %local_hash = ();
       		$local_hash{id} = $attribute->{attributeid};
       		$local_hash{description} = $attribute->{attributedescription};
       		push @descriptions, {%local_hash};
   		}
	}
    ...
    for my $cname (keys %$classes){
        my $class = { 
            ...
            classvisibility => 
                ...
                ? 'private' : 'public',
            classdescription => $classes->{$cname}->{constructor_vars}->{class}[0]
        };
    ...
    for my $k (qw(instance class)){
        for my $field (@{$class->{"${k}_fields"}}){
            ...
            my $attr = {
                ...
                attributedescription => $field->{field_description}
            };
