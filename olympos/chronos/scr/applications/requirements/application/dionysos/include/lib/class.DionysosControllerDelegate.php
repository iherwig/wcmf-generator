...
require_once(BASE."wcmf/application/controller/class.AssociateController.php");
...
class DionysosControllerDelegate
{
  ...
  function postInitialize(&$controller)
  {
    ...
    switch ($request->getAction())
    {
      ...
      case 'display':
        ...
        $request->setValue('omitMetaData', true);
        break;
      ...
      case 'disassociate':
        ...
        // determine if we have a parent or child relation
        $persistenceFacade = PersistenceFacade::getInstance();
        $parentTpl = $persistenceFacade->create($sourceType, 1);
        $childTpl = $persistenceFacade->create($targetType, 1);
        if (AssociateController::isDirectAssociation($parentTpl, $childTpl)) {
          $relationType = 'child';
        }
        elseif (AssociateController::isDirectAssociation($childTpl, $parentTpl)) {
          $relationType = 'parent';
        }
        else {
          $relationType = '';
        }
        $request->setValue('associateAs', $relationType);
        break;
  ...
  function postExecute(&$controller, $result)
  {
    ...
    switch ($request->getAction())
    {
      ...
      case 'display':
        // also load parents, if builddepth >= 1 (dionysos does not know about parents and children)
        $node = $response->getValue('node');
        if ($node && $response->getValue('depth') >= 1) {
          $node->loadParents(null);
        }
              ...
              if ($curChild->isManyToManyObject())
              {
                ...
                for ($j=0, $numParents=sizeof($parents); $j<$numParents; $j++)
                {
                  ...
                  if ($curParent->getBaseOID() != $curObj->getBaseOID()) {
                    ...
                    $curObj->_children[] = &$curParent;
                    $curObj->_properties['childoids'][] = $curParent->getOID();
