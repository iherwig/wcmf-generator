...
-->
<!-- PROTECTED REGION ID(application/include/views/ChiControllerView.tpl/Body) START -->
...
{if $viewMode == 'detail'}
    ...
    // ChiNode
    {assign var="template" value=$possiblechildren.ChiNode}
    ...
    grids['ChiNodeChild'] = new wcmf.grid.Grid();
    // column definitions
    var curColumnDefs = [];
    ...
    curColumnDefs.push({ldelim}id:"{$name}", dataIndex:"{$name}", header:"{$name}", width:{$columnWidth}, sortable:true, renderer:grids['ChiNodeChild'].renderColumnDefault.createDelegate(grids['ChiNodeChild']){rdelim});
    {/foreach}
    columDefs['ChiNodeChild'] = curColumnDefs;
    // button definitions
    var curButtonDefs = [];
    {if $template->getProperty('canCreate')}
      ...
      var dlgChiNodeChild = new AssociateDialog();
      curButtonDefs.push({ldelim}icon:'images/link.png', cls:'x-btn-icon', tooltip:{ldelim}text:'{translate text="Associate selected"}'{rdelim}, 
        handler:function(){ldelim}dlgChiNodeChild.show('{$realSubject->getType()}', grids['ChiNodeChild'], '{$poid}', 'child', false);{rdelim}{rdelim});
      {/if}
    {/if}
    buttonDefs['ChiNodeChild'] = curButtonDefs;
    // grid initialization
    grids['ChiNodeChild'].init('{$realSubject->getObjectDisplayName()}', 'ChiNode', '{$nodeUtil->getChildQuery($node, 'ChiNode')|replace:"'":"\'"}', columDefs['ChiNodeChild'], {ldelim}paging:true, autoheight:true, singleSelect:true, ddRows:{$ddRows}{rdelim}, [new wcmf.grid.EditAction(), {if $template->getProperty('canCreate')}new wcmf.grid.DuplicateAction(), {/if}new wcmf.grid.DeleteAction()], buttonDefs['ChiNodeChild'], {ldelim}poid:'{$node->getOID()}'{rdelim});
    grids['ChiNodeChild'].getGridImpl().applyToMarkup('ChiNodeChildGrid');
    grids['ChiNodeChild'].load();
...
<!-- ChiNode -->
<div class="contentblock">
  <div id="ChiNodeChildGrid" style="border:1px solid #99bbe8;overflow: hidden; width: 445px;"></div>
</div>
