...
-->
<!-- PROTECTED REGION ID(application/include/views/ChiRequirementView.tpl/Body) ENABLED START -->
...
{if $viewMode == 'detail'}
    ...
    {if $template->getProperty('canCreate')}
      ...
      curButtonDefs.push({ldelim}icon:'images/link.png', cls:'x-btn-icon', tooltip:{ldelim}text:'{translate text="Associate selected"}'{rdelim}, 
        handler:function(){ldelim}dlgChiIssueChild.show('{$realSubject->getType()}', grids['ChiIssueChild'], '{$poid}', 'child', false);{rdelim}{rdelim});
      {/if}
    {/if}
    buttonDefs['ChiIssueChild'] = curButtonDefs;
    // grid initialization
    grids['ChiIssueChild'].init('{$realSubject->getObjectDisplayName()}', 'ChiIssue', '{$obfuscator->obfuscate($nodeUtil->getChildQuery($node, 'ChiIssue'))}', columDefs['ChiIssueChild'], {ldelim}paging:true, autoheight:true, singleSelect:true, ddRows:{$ddRows}{rdelim}, [new wcmf.grid.EditAction(), {if $template->getProperty('canCreate')}new wcmf.grid.DuplicateAction(), {/if}new wcmf.grid.DeleteAction()], buttonDefs['ChiIssueChild'], {ldelim}poid:'{$node->getOID()}'{rdelim});
