...
/**
 * This file was generated by wCMFGenerator 3.0.0017 from requirements.uml on Wed Nov 18 12:43:49 CET 2009. 
...
class ActivityReceiveBase extends ChiBase
{
    ...
    /**
     * @deprecated use getARControlFlowSourceEndChildren() instead
     */
    function getARControlFlowSourceEndList()
    {
      Log::warn("use of deprecated method getARControlFlowSourceEndList. use getARControlFlowSourceEndChildren() instead.\n".WCMFException::getStackTrace(), __CLASS__);
      return $this->getARControlFlowSourceEndChildren();
    }
    function getARControlFlowSourceEndChildren()
    {
      return $this->getChildrenEx(null, 'ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID()), null, false);
    }
    /**
    ...
    /**
     * @deprecated use getARControlFlowTargetEndChildren() instead
     */
    function getARControlFlowTargetEndList()
    {
      Log::warn("use of deprecated method getARControlFlowTargetEndList. use getARControlFlowTargetEndChildren() instead.\n".WCMFException::getStackTrace(), __CLASS__);
      return $this->getARControlFlowTargetEndChildren();
    }
    function getARControlFlowTargetEndChildren()
    {
      return $this->getChildrenEx(null, 'ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID()), null, false);
    }
    /**
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'Activity')
      {
        ...
        $children = parent::getChildrenEx(null, 'ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID()), null, false);
      ...
      if ($type == 'Activity')
      {
        // for every ControlFlow we have to load the Activity 
        $this->loadChildren('ControlFlow');
        $children = parent::getChildrenEx(null, 'ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID()), null, false);
        $persistenceFacade = &PersistenceFacade::getInstance();
        $grandChildren = array();
        for($i=0; $i<sizeof($children); $i++)
        {
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
          $associationNode->setActivityReceive($this);
      ...
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityInitial')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_activityinitial_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityFinal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_activityfinal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityInitial')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_activityinitial_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityFinal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_activityfinal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $ids[0]);
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityInitial')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_activityinitial_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityFinal')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowsource_id' => $this->getDBID(), 'fk_activityfinal_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityInitial')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_activityinitial_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityFinal')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_arcontrolflowtarget_id' => $this->getDBID(), 'fk_activityfinal_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
