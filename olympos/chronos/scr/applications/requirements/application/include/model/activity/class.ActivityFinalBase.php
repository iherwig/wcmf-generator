...
/**
 * This file was generated by wCMFGenerator 3.0.0018 from requirements.uml on Mon Nov 30 13:40:36 CET 2009. 
 ...
 * @version 1.0
 */
class ActivityFinalBase extends ChiBase
{
    function ActivityFinalBase($oid=null, $type=null)
    {
      if ($type == null)
        parent::__construct($oid, 'ActivityFinal');
      else
    ...
    {
      return Message::get("ActivityFinal");
    }
    /**
    ...
    {
      return Message::get("An final t node is depicted by a cirlcled small black spot.");
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'ControlFlow', array('fk_activityfinal_id' => $this->getDBID()), null, false);
    }
    /**
    ...
    {
      return $this->getChildrenEx(null, 'Figure', array('fk_activityfinal_id' => $this->getDBID()), null, false);
    }
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'Activity')
      {
        ...
        $children = parent::getChildrenEx(null, 'ControlFlow', array('fk_activityfinal_id' => $this->getDBID()), null, false);
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
          $associationNode->setActivityFinal($this);
      ...
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityReceive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_arcontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityReceive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_arcontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityInitial')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityfinal_id' => $this->getDBID(), 'fk_activityinitial_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $ids[0]);
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityReceive')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_arcontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityReceive')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_arcontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityInitial')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityfinal_id' => $this->getDBID(), 'fk_activityinitial_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
