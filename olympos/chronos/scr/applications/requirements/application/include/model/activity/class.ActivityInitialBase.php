...
/**
 * This file was generated by wCMFGenerator 3.0.0018 from requirements.uml on Thu Nov 26 12:43:29 CET 2009. 
...
class ActivityInitialBase extends ChiBase
{
    ...
    {
      if ($type == null)
        parent::__construct($oid, 'ActivityInitial');
      else
    ...
    /**
     * @deprecated use getControlFlowChildren() instead
     */
    function getControlFlowList()
    {
      Log::warn("use of deprecated method getControlFlowList. use getControlFlowChildren() instead.\n".WCMFException::getStackTrace(), __CLASS__);
      return $this->getControlFlowChildren();
    }
    function getControlFlowChildren()
    {
      return $this->getChildrenEx(null, 'ControlFlow', array('fk_activityinitial_id' => $this->getDBID()), null, false);
    }
    /**
    ...
    function loadChildren($type, $buildDepth=BUILDDEPTH_SINGLE, $forceUpdate=false)
    {
      ...
      if ($type == 'Activity')
      {
        ...
        $children = parent::getChildrenEx(null, 'ControlFlow', array('fk_activityinitial_id' => $this->getDBID()), null, false);
    ...
    function addChild(&$child, $addtype=ADDCHILD_BACK)
    {
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
          $associationNode->setActivityInitial($this);
      ...
      if ($child != null && $child->getType() == 'Activity')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityDecision')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityReceive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_arcontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityReceive')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_arcontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivitySend')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
      ...
      if ($child != null && $child->getType() == 'ActivityFinal')
      {
        ...
        $persistenceFacade = &PersistenceFacade::getInstance();
        // check if the relation already exists
        $relOID = $persistenceFacade->getFirstOID('ControlFlow', array('fk_activityinitial_id' => $this->getDBID(), 'fk_activityfinal_id' => $child->getDBID()));
        if ($relOID == null)
        {
          // add the child only if it is not added already
          $associationNode = &$persistenceFacade->create('ControlFlow', BUILDTYPE_SINGLE);
    ...
    function deleteChild($childOID, $reallyDelete=false)
    {
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_acontrolflowtarget_id' => $ids[0]);
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'Activity')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_acontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_adcontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityDecision')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_adcontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityReceive')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_arcontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityReceive')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_arcontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_ascontrolflowtarget_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivitySend')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_ascontrolflowsource_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
      ...
      if (PersistenceFacade::isValidOID($childOID) && PersistenceFacade::getOIDParameter($childOID, 'type') == 'ActivityFinal')
      {
        ...
        // check if the connection child is loaded already
        $ids = PersistenceFacade::getOIDParameter($childOID, 'id');
        $associationNodeConstraint = array('fk_activityinitial_id' => $this->getDBID(), 'fk_activityfinal_id' => $ids[0]);
        $associationNode = &$this->getFirstChild('ControlFlow', $associationNodeConstraint, null, false);
        if ($associationNode != null)
          $childOID = $associationNode->getOID();
        else
