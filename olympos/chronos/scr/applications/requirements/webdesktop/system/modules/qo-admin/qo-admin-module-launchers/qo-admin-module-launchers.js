QoDesk.QoAdminModuleLaunchers = Ext.extend(Ext.app.Module, {

	  moduleType : 'system'
	, moduleId : 'qo-admin-module-launchers'
	, phpFile : 'qo-admin-module-launchers.php'
	...
	, init : function(){
        this.launcher = {
            ...
            iconCls: 'icon-grid',
            scope: this,
            text: 'QO Admin - Module Launchers',
            tooltip: '<b>QO Admin</b><br />Module Launcher Administration'
        }
    }
	...
	, createWindow : function(){
        ...
        var win = desktop.getWindow('qo-admin-module-launchers-win');
		...
		if (!win) {
			...
			// for Grid data
			var storeModuleLaunchers = new Ext.data.Store ({
			...
			var storeLauncherNames2 = new Ext.data.Store ({
				proxy: new Ext.data.HttpProxy ({ 
					url: this.app.connection
					, scope: this
				})
			...
			storeLauncherNames2.loadData;
			storeLauncherNames2.load();

			// End Store Definitions

			var fm = Ext.form;
			// Define column Model
			var cmModuleLaunchers = new Ext.grid.ColumnModel([
				{
					id:'id'
					, header: 'ID'
					, dataIndex: 'id'
				...
				, {	
					...
					, renderer: function(data) {
						record = storeLauncherNames.getById(data);
						if(record) {
							return record.data.DisplayField;
						} else {
							return 'missing data';
			...
			cmModuleLaunchers.defaultSortable = true;
	
			function handleDeleteModuleLaunchers() {
				//returns record objects for selected rows (all info for row)
				var selectedRows = editGridModuleLaunchers.selModel.selections.items;
				
				//returns array of selected rows ids only
				var selectedKeys = editGridModuleLaunchers.selModel.selections.keys; 
				...
				Ext.Ajax.request({
					...
					, success:function(response,options){
						storeModuleLaunchers.reload();
					}
					, scope: this
				});
			};
	
			function saveEditModuleLaunchers (oGrid_Event) {
				...
				Ext.Ajax.request({
					...
					, success:function(response,options){
						storeModuleLaunchers.commitChanges();
					}      
					, scope: this
				});
			};
			...
			// Grid definition
			var editGridModuleLaunchers = new Ext.grid.EditorGridPanel({
				store: storeModuleLaunchers
				, cm: cmModuleLaunchers
				, height: 150
				...
				, tbar: [
					...
					, {
						...
						, handler: handleDeleteModuleLaunchers
						, scope: this
					}
					, {
						text: 'Refresh'
			...
			editGridModuleLaunchers.addListener('afteredit', saveEditModuleLaunchers, this);

			// trigger the data store load
			storeModuleLaunchers.load();

			var newFormModuleLaunchers = new Ext.FormPanel ({
				title: 'New Module Launcher'
				, id: 'qo-admin-module-launcher-new'
				, labelWidth: 95
				, url: this.app.connection
				, frame: true
				...
				, tbar: [
					...
					, {
						...
						, type: 'submit'
						, handler: function () {
							newFormModuleLaunchers.getForm().submit({
								waitMsg: 'Saving...'
								, success: function (response,options) {
									newFormModuleLaunchers.getForm().reset();
								}
								, failure: function (response,options) {
			...
			var tabModules = new Ext.TabPanel ({
				...
				, items: [ 
					newFormModuleLaunchers
					, editGridModuleLaunchers
				]
			});

			win = desktop.createWindow({
                id: 'qo-admin-module-launchers-win',
                title:'QO Admin - Module Launchers',
                width:722,
                height:400,
                iconCls: 'icon-grid',
                shim:false,
				...
				layout: 'fit',
                items: tabModules,
                taskbuttonTooltip: '<b>QO Admin</b><br />Module Launcher Administration'
            });
		}
		win.show();
	}
